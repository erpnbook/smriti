# Foundation: Workspace Layout Fix

## 1. Purpose
Implementation of Workspace Layout Fix for SMRITI Retail OS.
...
3. **The SMRITI Conflict**: Since `smriti_retail_os` contains custom **Pages** and **Custom Fields** on standard doctypes, but **—ero custom DocTypes of its own**, the `"SMRITI Retail OS"` module was never added to the user's (even Administrator's) `allow_modules` list. This filtered out all the links, resulting in a blank workspace layout!
