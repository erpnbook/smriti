...
class TestPOSProfileManagement(unittest.TestCase):
    def setUp(self):
        ...
        self.cash_account = frappe.db.get_value("Account", {"company": self.company, "account_type": "Cash"}, "name")
        if not self.cash_account:
            self.cash_account = frappe.db.get_value("Account", {"company": self.company, "is_group": 0}, "name")
        ...
        if not frappe.db.exists("Mode of Payment", self.mop):
            ...
            mop_doc = frappe.get_doc("Mode of Payment", self.mop)
        ...
        shift_names = frappe.db.get_all("POS Opening Entry", {"pos_profile": ["in", [self.source_profile, self.clone_profile_name]]}, pluck="name")
        if shift_names:
            frappe.db.delete("POS Opening Entry Detail", {"parent": ["in", shift_names]})
            frappe.db.delete("POS Opening Entry", {"name": ["in", shift_names]})
        frappe.db.delete("POS Payment Method", {"parent": ["in", [self.source_profile, self.clone_profile_name]]})
        frappe.db.delete("POS Profile User", {"parent": ["in", [self.source_profile, self.clone_profile_name]]})
        frappe.db.delete("POS Profile", {"name": ["in", [self.source_profile, self.clone_profile_name]]})
        frappe.db.commit()
    ...
    def test_soft_delete_archiving(self):
        ...
        disabled_val = frappe.db.get_value("POS Profile", self.source_profile, "disabled")
