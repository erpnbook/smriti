frappe.pages['smriti-udne'].on_page_load = function(wrapper) {
    ...
    function setupEvents() {
        ...
        ['#rule-template', '#document-type', '#rule-priority', '#priority-value', '#reset-rule'].forEach(function(selector) {
            wrapper.querySelector(selector).addEventListener('input', function() {
                triggerPreview();
            });
            wrapper.querySelector(selector).addEventListener('change', function() {
                triggerPreview();
            });
        });
        
        wrapper.querySelector('#run-scanner-btn').addEventListener('click', function() {
            runScanner();
        });
    }
    ...
    function loadReservations() {
        frappe.call({
            method: "smriti_retail_os.api.udne_api.get_reservations",
            callback: function(r) {
                    ...
                    tbody.innerHTML = r.message.map(function(row) {
                        ...
                        return `
                            <tr>
                                ...
                                <td>${row.start_number} - ${row.end_number}</td>
                                <td>${row.current_counter}</td>
                                <td><span class="smriti-badge smriti-badge-${statusClass}">${row.status}</span></td>
                            </tr>
                        `;
                    }).join('');
                } else {
                    tbody.innerHTML = `<tr><td colspan="4" style="text-align: center; color: #64748b;">No active reservations found.</td></tr>`;
                }
            }
        });
    }
