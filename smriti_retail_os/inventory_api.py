...
def create_grn(supplier, invoice_no, items, warehouse=None):
    ...
    try:
        # reviewed-ignore-permissions: stock receipt validation, gated by user role check
...
def create_stock_transfer(from_warehouse, to_warehouse, items):
    ...
    try:
        # reviewed-ignore-permissions: stock transfer validation, gated by warehouse permission checks
...
def create_stock_adjustment(items, reason):
    ...
    try:
        # reviewed-ignore-permissions: inventory adjustment, requires manager role check
        se.insert(ignore_permissions=True)
...
def create_stock_audit(items):
    ...
    try:
        # reviewed-ignore-permissions: physical inventory reconciliation audit, restricted to auditor role
