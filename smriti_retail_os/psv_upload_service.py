...
def process_upload(upload_doc_name: str):
    ...
    try:
        ...
        smriti.errors.log_error(frappe.get_traceback(), f"PSV Upload Error: {upload_doc_name}")
...
def _check_duplicate_file(file_url: str, current_doc_name: str):
    ...
    smriti.db.set_value("PSV Sell-Through Upload", current_doc_name, "file_hash", file_hash)
...
def _parse_and_map_csv(file_path: str):
    ...
    barcode_map = {}
    for b in smriti.db.get_list("Item Barcode", fields=["barcode", "parent"]):
    ...
    with open(file_path, mode="r", encoding="utf-8-sig") as csvfile:
        ...
        for row_idx, row in enumerate(reader, start=2):  # Start at 2 (header = row 1)
            ...
            if not item_code:
                ...
                if smriti.db.exists("Item", raw_barcode):
...
def _fail_upload(doc, errors: list):
    ...
    smriti.db.rollback()  # Roll back any partial changes
...
def _commit_to_ledger(doc, parsed_rows: list):
    ...
    try:
        ...
        smriti.db.commit()  # Success — commit everything
        frappe.msgprint(
        ...
        smriti.db.rollback()  # Abort — revert all ledger entries
        smriti.errors.log_error(frappe.get_traceback(), f"PSV Atomic Commit Failed: {doc.name}")
