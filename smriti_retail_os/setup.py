...
def create_smriti_audit_event_doctype():
    ...
    try:
        ...
        smriti.errors.log_error(f"Error creating SMRITI Audit Event DocType: {str(e)}")
...
def create_smriti_attribute_layout_doctype():
    ...
    if smriti.db.exists("DocType", "SMRITI Attribute Layout"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Attribute Layout DocType: {str(e)}")
...
def create_smriti_company_settings_doctype():
    ...
    if smriti.db.exists("DocType", "SMRITI Company Settings"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Company Settings DocType: {str(e)}")
...
def create_reporting_doctypes():
    ...
    if smriti.db.exists("DocType", "SMRITI Report Template"):
        return
    # 1. SMRITI Report Role (Child DocType)
    if not smriti.db.exists("DocType", "SMRITI Report Role"):
        try:
            doc = smriti.documents.new("DocType")
            ...
            smriti.errors.log_error(f"Error creating SMRITI Report Role child doctype: {str(e)}")

    # 2. SMRITI Report Template
    if not smriti.db.exists("DocType", "SMRITI Report Template"):
        try:
            doc = smriti.documents.new("DocType")
            ...
            smriti.errors.log_error(f"Error creating SMRITI Report Template DocType: {str(e)}")
    else:
        try:
            dt = smriti.documents.get("DocType", "SMRITI Report Template")
            for f in dt.fields:
            ...
            smriti.errors.log_error(f"Error updating SMRITI Report Template options: {str(e)}")

    # 3. SMRITI Saved View
    if not smriti.db.exists("DocType", "SMRITI Saved View"):
        try:
            doc = smriti.documents.new("DocType")
            ...
            smriti.errors.log_error(f"Error creating SMRITI Saved View DocType: {str(e)}")
...
def seed_report_templates():
    ...
    for r in reports:
        ...
        # Check if exists
        if smriti.db.exists("SMRITI Report Template", key):
            doc = smriti.documents.get("SMRITI Report Template", key)
...
def create_audit_log_doctype():
    ...
    if smriti.db.exists("DocType", "SMRITI Address Audit Log"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Address Audit Log DocType: {str(e)}")



def create_master_doctypes():
    if smriti.db.exists("DocType", "SMRITI Heel Type"):
        return
    masters = [
    ...
    for doctype_name, label in masters:
        if not smriti.db.exists("DocType", doctype_name):
            try:
                ...
                smriti.errors.log_error(f"Error creating custom Master DocType {doctype_name}: {str(e)}")
...
def seed_master_doctypes():
    ...
    for doctype_name, values in seeds.items():
        if smriti.db.exists("DocType", doctype_name):
            for val in values:
                if not smriti.db.exists(doctype_name, val):
                    try:
                        ...
                        smriti.errors.log_error(f"Error seeding {val} to {doctype_name}: {str(e)}")
    ...
    if smriti.db.exists("DocType", "SMRITI Print Template"):
        ...
        for t in default_templates:
            if smriti.db.exists("SMRITI Print Template", t["name"]):
                try:
                    # Update path to refresh missing or updated fields on existing template
                    doc = smriti.documents.get("SMRITI Print Template", t["name"])
                    updated = False
                    ...
                    smriti.errors.log_error(f"Error updating print template {t['name']}: {str(e)}")
            else:
                try:
                    ...
                    smriti.errors.log_error(f"Error seeding print template {t['name']}: {str(e)}")
    smriti.db.commit()
...
def backup_and_seed_existing_data():
    ...
    for field, dt in field_to_doctype.items():
        if smriti.db.exists("DocType", dt) and frappe.db.has_column("Item", field):
            try:
                unique_vals = smriti.db.sql(f"select distinct `{field}` from `tabItem` where `{field}` is not null and `{field}` != ''", as_list=True)
                for (val,) in unique_vals:
                    val_clean = str(val).strip()
                    if val_clean and not smriti.db.exists(dt, val_clean):
                        try:
                            doc = smriti.documents.new(dt)
                            doc.attribute_value = val_clean
                            doc.insert(ignore_permissions=True)
                        except Exception as e:
                            smriti.errors.log_error(f"Error backing up {val_clean} to {dt}: {str(e)}")
            except Exception as e:
                smriti.errors.log_error(f"Error reading column {field} from Item: {str(e)}")
def setup_activity_log_options():
    ...
    try:
        ...
        smriti.db.delete("Property Setter", {"doc_type": "Activity Log", "field_name": "operation", "property": "options"})
        ...
        smriti.errors.log_error(f"Error updating Activity Log options: {str(e)}")
...
def create_smriti_key_custodian_doctype():
    ...
    try:
        ...
        smriti.errors.log_error(f"Error creating SMRITI Key Custodian DocType: {str(e)}")
...
def create_smriti_print_job_doctype():
    ...
    if smriti.db.exists("DocType", "SMRITI Print Job"):
        return
    ...
    try:
        ...
        smriti.errors.log_error(f"Error creating SMRITI Print Job DocType: {str(e)}")


def create_smriti_loyalty_tier_doctype():
    """Creates the SMRITI Loyalty Tier DocType."""
    if smriti.db.exists("DocType", "SMRITI Loyalty Tier"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Loyalty Tier DocType: {str(e)}")


def create_smriti_loyalty_rule_doctype():
    """Creates the SMRITI Loyalty Rule DocType."""
    if smriti.db.exists("DocType", "SMRITI Loyalty Rule"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Loyalty Rule DocType: {str(e)}")

def create_smriti_settings_doctype():
    """Creates the SMRITI Settings single DocType for global configurations."""
    if smriti.db.exists("DocType", "SMRITI Settings"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Settings DocType: {str(e)}")


def create_smriti_cge_settings_doctype():
    """Creates the SMRITI CGE Settings single DocType."""
    if smriti.db.exists("DocType", "SMRITI CGE Settings"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI CGE Settings DocType: {str(e)}")
...
def create_smriti_barcode_settings_doctype():
    ...
    try:
        if smriti.db.exists("DocType", "SMRITI Barcode Settings"):
            settings = smriti.documents.get_single("BarcodeSettings")
            updated = False
            defaults = {
        ...
        smriti.errors.log_error(title="Error seeding defaults for SMRITI Barcode Settings", message=str(e))
...
def create_smriti_barcode_scan_event_doctype():
    ...
    if smriti.db.exists("DocType", "SMRITI Barcode Scan Event"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(title="Error creating SMRITI Barcode Scan Event DocType", message=str(e))


def create_smriti_barcode_telemetry_snapshot_doctype():
    """Creates the SMRITI Barcode Telemetry Snapshot custom DocType."""
    if smriti.db.exists("DocType", "SMRITI Barcode Telemetry Snapshot"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(title="Error creating SMRITI Barcode Telemetry Snapshot DocType", message=str(e))


def create_smriti_trial_health_snapshot_doctype():
    """Creates the SMRITI Trial Health Snapshot custom DocType."""
    if smriti.db.exists("DocType", "SMRITI Trial Health Snapshot"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(title="Error creating SMRITI Trial Health Snapshot DocType", message=str(e))
...
def seed_trial_health_formula():
    ...
    if smriti.db.exists("SMRITI Formula Definition", {"formula_id": formula_id, "formula_version": formula_version}):
        return
    ...
    try:
        ...
        smriti.errors.log_error(title="Error seeding Trial Health Formula", message=str(e))
...
def create_smriti_wallet_ledger_doctype():
    ...
    try:
        ...
        smriti.errors.log_error(f"Error creating SMRITI Wallet Ledger DocType: {str(e)}")
...
def create_smriti_coupon_campaign_doctype():
    ...
    try:
        ...
        smriti.errors.log_error(f"Error creating SMRITI Coupon Campaign DocType: {str(e)}")


def create_smriti_liability_snapshot_doctype():
    """Creates the SMRITI Liability Snapshot custom DocType."""
    if smriti.db.exists("DocType", "SMRITI Liability Snapshot"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Liability Snapshot DocType: {str(e)}")


def create_smriti_rule_evaluation_log_doctype():
    """Creates the SMRITI Rule Evaluation Log custom DocType."""
    if smriti.db.exists("DocType", "SMRITI Rule Evaluation Log"):
        return
    try:
        doc = smriti.documents.new("DocType")
        ...
        smriti.errors.log_error(f"Error creating SMRITI Rule Evaluation Log DocType: {str(e)}")
    ...
    for dt in doctypes:
        if not smriti.db.exists("DocType", dt):
            ...
            try:
                ...
                smriti.errors.log_error(f"Error bootstrapping CGE v2 DocType {dt}: {str(e)}")
            ...
            if frappe.db.table_exists(doctype):
                ...
                for idx_name, cols in indexes.items():
                    ...
                    if not idx_rows:
                        ...
                        print(msg)
                        smriti.errors.log_error(msg, "SMRITI CGE Index Verification")
                    else:
...
def create_udne_doctypes():
    ...
    if not smriti.db.exists("DocType", "SMRITI Numbering Rule"):
        try:
            doc = smriti.documents.new("DocType")
            ...
            smriti.errors.log_error(f"Error creating SMRITI Numbering Rule DocType: {str(e)}")

    # 2. SMRITI Numbering Counter
    if not smriti.db.exists("DocType", "SMRITI Numbering Counter"):
        try:
            doc = smriti.documents.new("DocType")
            ...
            smriti.errors.log_error(f"Error creating SMRITI Numbering Counter DocType: {str(e)}")

    # 3. SMRITI Numbering Reserved Range
    if not smriti.db.exists("DocType", "SMRITI Numbering Reserved Range"):
        try:
            doc = smriti.documents.new("DocType")
            ...
            smriti.errors.log_error(f"Error creating SMRITI Numbering Reserved Range DocType: {str(e)}")

    # 4. SMRITI Numbering Audit Log
    if not smriti.db.exists("DocType", "SMRITI Numbering Audit Log"):
        try:
            doc = smriti.documents.new("DocType")
            ...
            smriti.errors.log_error(f"Error creating SMRITI Numbering Audit Log DocType: {str(e)}")


def _seed_root_item_group():
    """Ensure ERPNext root Item Group exists for SMRITI Category Master."""
    if not smriti.db.exists("Item Group", "All Item Groups"):
...
def setup_smriti_retail_os():
    ...
    for role_name in ["SMRITI Cashier", "SMRITI Store Manager", "SMRITI Administrator", "SMRITI Marketing Manager", "SMRITI Auditor"]:
        if not smriti.db.exists("Role", role_name):
            try:
                role = smriti.documents.new("Role")
                ...
                smriti.errors.log_error(f"Error creating role {role_name}: {str(e)}")
    ...
    for dt, fields in custom_fields.items():
        for f in fields:
            ...
            if smriti.db.exists("Custom Field", custom_field_name):
                smriti.db.set_value("Custom Field", custom_field_name, "module", "SMRITI Retail OS")
    ...
    if smriti.db.exists("Workspace", workspace_name):
        ws = smriti.documents.get("Workspace", workspace_name)
        ws.links = []
        ...
        ws = smriti.documents.new("Workspace")
    ...
    smriti.db.set_value("Workspace", workspace_name, "module", "Selling")
    smriti.db.set_value("Workspace", workspace_name, "public", 1)
    ...
    for dt, roles in doctype_permissions.items():
        for role, perm in roles.items():
            if not smriti.db.exists("Custom DocPerm", {"parent": dt, "role": role}):
                p = smriti.documents.new("CustomDocPerm")
                p.update({
                    "parent": dt,
                    "parenttype": "DocType",
...
def _setup_module_profiles():
    ...
    for profile_name, blocked in profiles.items():
        if not smriti.db.exists("Module Profile", profile_name):
            doc = smriti.documents.new("Module Profile")
    ...
    for profile_name, roles in role_profiles.items():
        if not smriti.db.exists("Role Profile", profile_name):
            doc = smriti.documents.new("Role Profile")
...
def hide_non_retail_modules():
    ...
    for ws_name in NON_RETAIL_WORKSPACES:
        if smriti.db.exists("Workspace", ws_name):
            smriti.db.set_value(
    ...
    try:
        ...
        smriti.errors.log_error(f"Error seeding default formulas: {str(e)}", "SMRITI Setup Error")

    try:
        from smriti_retail_os.patches.seed_default_terms import execute as seed_terms
        seed_terms()
    except Exception as e:
        smriti.errors.log_error(f"Error seeding default terms: {str(e)}", "SMRITI Setup Error")

    try:
        from smriti_retail_os.patches.seed_printability_formula import execute as seed_printability
        seed_printability()
    except Exception as e:
        smriti.errors.log_error(f"Error seeding printability formula: {str(e)}", "SMRITI Setup Error")

    try:
        from smriti_retail_os.patches.seed_telemetry_meta import execute as seed_telemetry
        seed_telemetry()
    except Exception as e:
        smriti.errors.log_error(f"Error seeding telemetry formula: {str(e)}", "SMRITI Setup Error")
...
def patch_company_tax_invoice(print_format_name=None):
    ...
    if not smriti.db.exists("Print Format", print_format_name):
        print(f"[SMRITI] Error: Print Format '{print_format_name}' does not exist!")
        return
        
    pf = smriti.documents.get("Print Format", print_format_name)
...
def create_default_admin_account():
    ...
    if not smriti.db.exists("User", email):
        try:
            ...
            smriti.errors.log_error(f"Error creating default Admin account: {str(e)}", "SMRITI Setup Error")
...
def seed_retail_defaults():
    ...
    for comp in smriti.db.get_list("Company", pluck="name"):
        has_address = smriti.db.exists("Address", {"links.link_doctype": "Company", "links.link_name": comp})
        if not has_address:
            try:
                abbr = smriti.db.get("Company", comp, "abbr") or "CO"
                ...
                if not smriti.db.exists("Address", addr_name):
                    ...
                    gstin = smriti.db.get("Company", comp, "gstin")
                    if gstin:
                        addr.gstin = gstin
                ...
                smriti.errors.log_error(f"Error seeding default Address for Company {comp}: {str(e)}", "SMRITI Setup Error")

    # 1. Walk-In Customer
    if not smriti.db.exists("Customer", "Walk-In Customer"):
        try:
            # Resolve non-group Customer Group
            cg = smriti.db.get("Customer Group", {"is_group": 0}, "name")
            if not cg:
                if not smriti.db.exists("Customer Group", "All Customer Groups"):
                    cg_parent = smriti.documents.new("Customer Group")
                    cg_parent.customer_group_name = "All Customer Groups"
                    cg_parent.is_group = 1
                    cg_parent.insert(ignore_permissions=True)
                cg_doc = smriti.documents.new("Customer Group")
            ...
            smriti.errors.log_error(f"Error seeding Walk-In Customer: {str(e)}", "SMRITI Setup Error")
    ...
    for mop in mops:
        if not smriti.db.exists("Mode of Payment", mop):
            try:
                ...
                smriti.errors.log_error(f"Error creating Mode of Payment {mop}: {str(e)}", "SMRITI Setup Error")
        ...
        try:
            ...
            for comp in smriti.db.get_list("Company", pluck="name"):
                ...
                if not has_mapping:
                    ...
                    ledger = None
                    if mop == "Cash":
                        ledger = f"Cash - {smriti.db.get('Company', comp, 'abbr')}"
                        if not smriti.db.exists("Account", ledger):
                            ledger = smriti.db.get("Account", {"account_name": "Cash", "company": comp})
                    else:
                        ledger = f"Bank - {smriti.db.get('Company', comp, 'abbr')}"
                        if not smriti.db.exists("Account", ledger):
                            ledger = smriti.db.get("Account", {"account_type": "Bank", "company": comp})
                            if not ledger:
                                ledger = smriti.db.get("Account", {"account_name": ["like", "%Bank%"], "company": comp, "is_group": 0})
            ...
            for acc in mop_doc.accounts:
                ...
                if comp_key not in seen_companies and smriti.db.exists("Company", acc.company):
            ...
            smriti.errors.log_error(f"Error mapping accounts for Mode of Payment {mop}: {str(e)}", "SMRITI Setup Error")
...
def ensure_installed_applications_setup_complete():
    ...
    try:
        ...
        smriti.errors.log_error(f"Error in ensure_installed_applications_setup_complete: {str(e)}")
