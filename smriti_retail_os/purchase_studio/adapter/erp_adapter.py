...
def get_default_warehouse(company):
    ...
    if not wh:
        wh = smriti.db.get("Warehouse", {"company": company}, "name")
...
def resolve_company(po_name=None, warehouse=None):
    ...
    company = None
    if po_name:
        company = smriti.db.get("Purchase Order", po_name, "company")
    if not company:
        company = frappe.defaults.get_user_default("company")
    if not company:
        company = smriti.db.get_single("Global Defaults", "default_company")
    if not company:
        all_cos = smriti.db.get_list("Company", limit=1, fields=["name"])
...
def list_purchase_orders(filters, fields, page=1, page_size=50):
    ...
    total = smriti.db.count("Purchase Order", filters)
...
def get_po(po_name):
    ...
    # ERPNext v14+: smriti.documents.get("Purchase Order", name)
    """
    if not smriti.db.exists("Purchase Order", po_name):
        frappe.throw(f"Purchase Order '{po_name}' not found.", frappe.DoesNotExistError)
    return smriti.documents.get("Purchase Order", po_name)
...
def get_or_create_batch(item_code, expiry_date=None):
    ...
    batch = smriti.documents.new("Batch")
...
def list_grns(filters, fields, page=1, page_size=50):
    ...
    limit_start = (page - 1) * page_size
    total = smriti.db.count("Purchase Receipt", filters)
    items = smriti.db.get_list(
        "Purchase Receipt",
        filters=filters,
        fields=fields,
...
def get_grn(grn_name):
    ...
    # ERPNext v14+: smriti.documents.get("Purchase Receipt", name)
    """
    if not smriti.db.exists("Purchase Receipt", grn_name):
        frappe.throw(f"Purchase Receipt '{grn_name}' not found.", frappe.DoesNotExistError)
    return smriti.documents.get("Purchase Receipt", grn_name)
...
def list_purchase_invoices(filters, fields, page=1, page_size=50):
    ...
    limit_start = (page - 1) * page_size
    total = smriti.db.count("Purchase Invoice", filters)
    items = smriti.db.get_list(
        "Purchase Invoice",
        filters=filters,
        fields=fields,
...
def get_pi(pi_name):
    ...
    # ERPNext v14+: smriti.documents.get("Purchase Invoice", name)
    """
    if not smriti.db.exists("Purchase Invoice", pi_name):
        frappe.throw(f"Purchase Invoice '{pi_name}' not found.", frappe.DoesNotExistError)
    return smriti.documents.get("Purchase Invoice", pi_name)
...
def supplier_exists(supplier):
    ...
    """
    return bool(smriti.db.exists("Supplier", supplier))
...
def get_supplier_doc(supplier):
    ...
    """
    return smriti.documents.get("Supplier", supplier)
...
def get_supplier_name(supplier):
    ...
    """
    return smriti.db.get("Supplier", supplier, "supplier_name")
...
def create_purchase_order_document():
    ...
    """
    return smriti.documents.new("Purchase Order")
...
def set_po_approval_status(po_name, status, reason=None):
    ...
    smriti.db.set_value("Purchase Order", po_name, values)
...
def purchase_order_exists(po_name):
    ...
    """
    return bool(smriti.db.exists("Purchase Order", po_name))
...
def get_po_supplier(po_name):
    ...
    """
    return smriti.db.get("Purchase Order", po_name, "supplier")
...
def create_purchase_receipt_document():
    ...
    """
    return smriti.documents.new("Purchase Receipt")
...
def grn_exists(grn_name):
    ...
    """
    return bool(smriti.db.exists("Purchase Receipt", grn_name))
...
def get_grn_docstatus(grn_name):
    ...
    """
    return smriti.db.get("Purchase Receipt", grn_name, "docstatus")
...
def create_purchase_invoice_document():
    ...
    """
    return smriti.documents.new("Purchase Invoice")
...
def auto_create_item_if_missing(item_code, rate, file_url=None):
    ...
    """
    if smriti.db.exists("Item", item_code):
        return
    ...
    default_group = smriti.db.get_single("SMRITI Settings", "default_item_group") or "Products"
    item.item_group           = default_group if smriti.db.exists("Item Group", default_group) else "Products"
    item.stock_uom            = "Nos"
    ...
    default_hsn = smriti.db.get_single("SMRITI Settings", "default_hsn_code")
    if default_hsn and smriti.db.exists("GST HSN Code", default_hsn):
    ...
    template_name = smriti.db.get("Item Tax Template", {"name": ["like", "%18%"]}, "name")
    if template_name:
    ...
    for pl_name, pl_rate in [("Standard Selling", rate * 1.2), ("MRP", rate * 1.5)]:
        existing_ip = smriti.db.get(
            "Item Price", {"item_code": item_code, "price_list": pl_name}, "name"
        )
        if existing_ip:
            smriti.db.set_value("Item Price", existing_ip, "price_list_rate", flt(pl_rate))
        else:
...
def list_landed_cost_vouchers(company, from_date=None, to_date=None, limit_start=0, limit_page_length=50):
    ...
    total = smriti.db.count("Landed Cost Voucher", filters)
    items = smriti.db.get_list(
        "Landed Cost Voucher",
        filters=filters,
...
def create_landed_cost_voucher_document():
    ...
    """
    return smriti.documents.new("Landed Cost Voucher")
...
def check_item_has_batch(item_code):
    ...
    """
    return bool(smriti.db.get("Item", item_code, "has_batch_no"))
...
def get_or_create_bridge_supplier(smriti_supplier_name):
    ...
    """
    smriti_supplier = smriti.documents.get("SMRITI Supplier", smriti_supplier_name)
    linked = smriti_supplier.get("erpnext_supplier")

    if linked and smriti.db.exists("Supplier", linked):
    ...
    erp_supplier = smriti.documents.new("Supplier")
...
def _get_default_supplier_group():
    return smriti.db.get("Supplier Group", {"is_group": 0}, "name") \
        or smriti.db.get("Supplier Group", {}, "name")
