...
def get_context(context):
    ...
    # Role guard — Barcode is accessible to Cashiers, Store Managers, and Admins
    roles = frappe.get_roles(frappe.session.user)
    allowed_roles = ["SMRITI Cashier", "SMRITI Store Manager", "System Manager"]
    if not any(r in roles for r in allowed_roles):
        frappe.throw(
            "Access Denied: Barcode Printing is restricted to Cashiers, Store Managers, and System Managers.",
