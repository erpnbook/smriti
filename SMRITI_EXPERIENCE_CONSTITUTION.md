...
Every business document generated by SMRITI (invoices, orders, receipts, labels, challans) is a user-facing surface governed by this constitution. A document is not merely a printout — it is a brand-carrying artefact.

### Rule DOC-E1 — Every Business Document Has Five Format Definitions

Each document listed below must have all five format views defined before it can be considered complete:

| Format | Channel | Purpose |
|---|---|---|
| **Screen View** | Browser / Web | Operational view; the user reads and acts on this |
| **Print View** | Browser Print | Optimised for A4 / thermal paper; no nav chrome |
| **PDF** | Download / Email attachment | Archival and statutory format |
| **Email** | Outbound email body | Inline HTML; mobile-friendly |
| **Mobile View** | Responsive / PWA | Thumb-readable summary on small screens |
...
| Purchase Invoice (Bill) | Purchase |
| Goods Receipt Note (GRN) | Purchase |
| Supplier Return | Purchase |
| Sales Invoice (Bill) | Billing |
| Sales Order / Quotation | Sales |
| Sales Return | Billing |
| Delivery Challan | Logistics |
| Stock Entry | Inventory |
| Stock Transfer | Inventory |
| Barcode Label | Label Studio |
| Customer Ledger | Reports |
| Supplier Ledger | Reports |

Until a document's Print Studio entry is complete, the Screen View is the only required format. PDF and Email formats are required before GA.

### Rule DOC-E2 — Documents Use SMRITI Branding

Every printed or emailed document must carry:
- SMRITI-formatted header (company name, logo, address, GSTIN)
- SMRITI typography (Inter body, Outfit headings)
- SMRITI colour palette (Navy #1A2B5C, Blue #2563EB)
- No Platform Engine footer, watermark, or reference

### Rule DOC-E3 — Document Language is Business Language

Document field labels must use retail language:

| Use This | Not This |
|---|---|
| Bill No. | Sales Invoice ID |
| Order No. | Purchase Order Name |
| Stock Movement | Stock Ledger Entry |
...
The Print View must contain only the document. No sidebar, no topbar, no breadcrumb, no action buttons. Use `@media print` rules or a dedicated `smriti-po-print.html`-style page.

### Rule DOC-E5 — Documents Are Registered

Every document format must be registered in the SMRITI Print Studio (`/print_templates`). No ad-hoc HTML-to-PDF conversion outside the Print Framework is permitted.

*See `SMRITI_UI_ARCHITECTURE.md` §Document Format Matrix for the full specification. See SPC-C-013 in `SMRITI_PRODUCT_CONSTITUTION.md` for governance status.*
